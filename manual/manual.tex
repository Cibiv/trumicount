...
TRUmiCount relies on UMI-Tools\footnote{Smith, T.S. \textit{et al.} UMI-tools: Modelling sequencing errors in Unique Molecular Identifiers to improve quantification accuracy. \textit{Genome Research} \textbf{27}, 491-499 (2017). \url{http://github.com/CGATOxford/UMI-tools}} to extract the UMIs associated with a gene (or other genomic feature) from the mapped reads in a BAM file. You can install UMI-tools with
\begin{shellcode}
conda install umi_tools
\end{shellcode}
We alternatively offer a modified version of UMI-Tools that improve the handling of paired-end reads, and which can be installed with
\begin{shellcode}
conda install -c http://www.cibiv.at/~pflug_/conda/ umi_tools
\end{shellcode}
