...
\item[Variance of the gene-specific loss estimate.] Shows the observed variance of the gene-specific loss estimates as a function of the number of UMIs per gene ($n^{\textrm{\tiny obs}}$) and compares it to the interpolated curve ($s + u / n^{\textrm{\tiny obs}}$).
\end{description}
...
This writes the bias-corrected counts to \texttt{kv\_1000g.tab}, and outputs the following diagnostic plots to \texttt{kv\_1000g.pdf}.
